...
export class Mobiledeviceinvitations {
    ...
    async findMobileDeviceInvitations(
        ...
        const operationUrl: string = baseURL.replace(/\/$/, "") + "/mobiledeviceinvitations";
        const client: AxiosInstance = this.sdkConfiguration.defaultClient;
        let globalSecurity = this.sdkConfiguration.security;
        if (typeof globalSecurity === "function") {
            globalSecurity = await globalSecurity();
        ...
        switch (true) {
            case httpRes?.status == 200:
                if (utils.matchContentType(contentType, `application/json`)) {
                    res.twoHundredApplicationJsonMobileDeviceInvitations = [];
                    const resFieldDepth: number = utils.getResFieldDepth(res);
                    res.twoHundredApplicationJsonMobileDeviceInvitations = utils.objectToClass(
