...
export class Advancedmobiledevicesearches {
    ...
    async findAdvancedMobileDeviceSearches(
        ...
        const operationUrl: string = baseURL.replace(/\/$/, "") + "/advancedmobiledevicesearches";
        const client: AxiosInstance = this.sdkConfiguration.defaultClient;
        let globalSecurity = this.sdkConfiguration.security;
        if (typeof globalSecurity === "function") {
            globalSecurity = await globalSecurity();
        ...
        switch (true) {
            case httpRes?.status == 200:
                if (utils.matchContentType(responseContentType, `application/json`)) {
                    res.twoHundredApplicationJsonAdvancedMobileDeviceSearches = [];
                    const resFieldDepth: number = utils.getResFieldDepth(res);
                    res.twoHundredApplicationJsonAdvancedMobileDeviceSearches = utils.objectToClass(
