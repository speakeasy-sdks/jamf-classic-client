...
export class Patchpolicies {
    ...
    async deletePatchPolicyById(
        ...
        const operationUrl: string = utils.generateURL(baseURL, "/patchpolicies/id/{id}", req);
        const client: AxiosInstance = this.sdkConfiguration.defaultClient;
        let globalSecurity = this.sdkConfiguration.security;
        if (typeof globalSecurity === "function") {
            globalSecurity = await globalSecurity();
    ...
    async findPatchPolicies(
        ...
        const operationUrl: string = baseURL.replace(/\/$/, "") + "/patchpolicies";
        const client: AxiosInstance = this.sdkConfiguration.defaultClient;
        let globalSecurity = this.sdkConfiguration.security;
        if (typeof globalSecurity === "function") {
            globalSecurity = await globalSecurity();
        ...
        switch (true) {
            case httpRes?.status == 200:
                if (utils.matchContentType(contentType, `application/json`)) {
                    res.twoHundredApplicationJsonPatchPolicies = [];
                    const resFieldDepth: number = utils.getResFieldDepth(res);
                    res.twoHundredApplicationJsonPatchPolicies = utils.objectToClass(
